...
def record_delivery(conn, delivery_data):
    ...
    cursor.execute(
        "INSERT INTO deliveries (delivery_id, delivery_date, order_id) VALUES (?, ?, ?)",
        (delivery_id, delivery_date, order_id),
    )
    ...
    for item in delivered_items:
        ...
        if not order_item or order_item[0] < quantity:
            raise ValueError(
                f"Invalid delivery quantity for item {item_id} in order {order_id}"
            )
    ...
    cursor.execute(
        "INSERT INTO delivery_tracking (delivery_id, tracking_date, status) VALUES (?, ?, 'in_transit')",
