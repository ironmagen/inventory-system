...
def adjustInventory(adjustments, userId=None):
  ...
  for adjustment in adjustments:
    ...
    cursor.execute('UPDATE items SET quantity = ? WHERE item_id = ?', (newQuantity, itemId))
    ...
    cursor.execute('''
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    ''', (userId, itemId, currentQuantity, newQuantity, quantityDifference, category, vendor))

    # Update category and vendor totals
    if category:
      cursor.execute('UPDATE category_totals SET total_quantity = total_quantity + ? WHERE category_id = ?', (quantityDifference, category))
      cursor.execute('INSERT OR IGNORE INTO category_totals (category_id, total_quantity) VALUES (?, ?)', (category, quantityDifference))
    if vendor:
      cursor.execute('UPDATE vendor_totals SET total_quantity = total_quantity + ? WHERE vendor_id = ?', (quantityDifference, vendor))
      cursor.execute('INSERT OR IGNORE INTO vendor_totals (vendor_id, total_quantity) VALUES (?, ?)', (vendor, quantityDifference))
