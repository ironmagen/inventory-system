...
def place_order(conn, order_data):
  ...
  # Validate order data
  if not order_data.get('items') or not order_data.get('vendor_id') or not order_data.get('order_date'):
  ...
  cursor.execute("INSERT INTO orders (order_id, order_date, vendor_id, status) VALUES (?, ?, ?, 'pending')", (order_id, order_date, vendor_id, 'pending'))
  ...
  for item in order_data['items']:
    ...
    cursor.execute("INSERT INTO order_items (order_id, item_id, quantity, price) VALUES (?, ?, ?, ?)", (order_id, item_id, quantity, price))
