...
def record_sales(sales_data):
  ...
  insert_sales_item = cursor.prepare('INSERT INTO sales_items (sale_id, item_id, quantity, recipe_id) VALUES (?, ?, ?, ?)')
  update_item_quantity = cursor.prepare('UPDATE items SET quantity = quantity - ? WHERE item_id = ?')
  ...
  for sale in sales_data:
    ...
    for sales_item in sales_items:
      ...
      if recipe_id:
        ...
        cursor.execute('SELECT ingredient_id, quantity FROM recipe_ingredients WHERE recipe_id = ?', (recipe_id,))
        ...
        for ingredient in ingredients:
          ...
          cursor.execute('UPDATE items SET quantity = quantity - ? WHERE item_id = ?', (ingredient_consumption, ingredient_id))
      else:
